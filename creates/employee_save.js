 ...
 */
const saveEmployee = async (z, bundle) => {
  ...
  try {
    ...
    // determine if the EMPLOYEE record exists
    const search = await intacct.get_objects_by_name(client, 'EMPLOYEE', [bundle.inputData.employeeId], ['RECORDNO','EMPLOYEEID'])
        
    let keys = null
    if (search.length) {
      // successful EmployeeUpdate returns RECORDNO and EMPLOYEEID
      keys = await intacct.employees.update_employee(client, bundle.inputData)
    }
    else {
      // successful EmployeeCreate returns RECORDNO and EMPLOYEEID
      keys = await intacct.employees.create_employee(client, bundle.inputData)
    }
...
module.exports = {
  ...
  display: {
    label: 'Save Employee',
    description: 'Save an Intacct employee (create or update).',
    hidden: false,
    important: true,
  },
  operation: {
